...
def insertname(username):
   cur.execute(
      "INSERT INTO user_score VALUES('{}', 0, 0)".format(username)
   )

def savegame(user , score , level):
   cur.execute(
      "SELECT * FROM user_score WHERE username = '{}'".format(user)
   )
   row = cur.fetchone()
   cur.execute(
      "UPDATE user_score SET user_score = '{}', user_level = '{}' WHERE username = '{}'".format (max(row[1], score), max(row[2], level), user)
   )


print("Enter your name ")
username = input()
cur.execute("SELECT count(*) FROM user_score WHERE username='{}'".format(username))

if cur.fetchone()[0] == 0:
   insertname(username)
else:
      cur.execute("SELECT * FROM user_score WHERE username = '{}'".format(username))
      data=cur.fetchone()
            ...
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_q:
                        game_over = True
                        game_close = False
                        savegame(username , Length_of_snake , level)
        ...
        while pause:   
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN and event.key == pygame.K_u:
                    pause = False
            unpause = font_style.render('Press u to unpause', True, 'white')
            dis.blit(unpause, unpause.get_rect(center = (dis_width // 2, dis_height // 2)))
            pygame.display.update()
